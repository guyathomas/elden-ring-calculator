...
import { loadCompressedData, USE_MSGPACK, getDecompressedBytes, parseData } from './loadData';
 ...
 */
export const enemyData: PrecomputedEnemyData = enemyDataJson as PrecomputedEnemyData;
 ...
 */
export const weaponDamageTypes: Record<string, PhysDamageType[]> = weaponDamageTypesData as Record<string, PhysDamageType[]>;
 ...
 */
export const weaponCombos: Record<string, Array<[number, number]>> = weaponCombosData as Record<string, Array<[number, number]>>;

/**
 * Get the number of true combos available for a weapon at a given target poise.
 * Returns 0 if weapon has no combo data.
 */
export function getWeaponTrueCombos(weaponName: string, targetPoise: number): number {
 ...
 */
export const weaponsUniqueAttacks: Record<string, boolean> = weaponsUniqueAttacksData as Record<string, boolean>;
...
export const classStandardAnimations: ClassStandardAnimations = classStandardAnimationsData as ClassStandardAnimations;

/**
 * Get the standard animation ID for a weapon class and attack suffix.
 * Returns null if no standard found.
 */
export function getClassStandardAnimation(wepType: number, attackSuffix: string): string | null {
 ...
 */
export const weaponDpsData: Record<string, WeaponDpsData> = weaponDpsDataJson as Record<string, WeaponDpsData>;
...
export async function loadPrecomputedData(): Promise<PrecomputedDataV2> {
  ...
  cachedData = await loadCompressedData<PrecomputedDataV2>(msgpackDataUrl, jsonGzippedDataUrl);
  return cachedData;
}
...
export async function loadAowData(): Promise<PrecomputedAowData> {
  ...
  cachedAowData = await loadCompressedData<PrecomputedAowData>(aowMsgpackDataUrl, aowJsonGzippedDataUrl);
...
import type { AnimationIndexEntry, AnimationEventData, ComboData } from '../types';
...
const animationChunkCache = new Map<string, Promise<Record<string, AnimationEventData>>>();
 ...
 */
function loadAnimationChunk(prefix: string): Promise<Record<string, AnimationEventData>> {
  ...
  if (!promise) {
    ...
    promise = fetch(url).then(async (response) => {
      if (!response.ok) throw new Error(`Failed to load animation chunk ${prefix}: ${response.status}`);
 ...
 */
export async function fetchAnimationData(animationId: string): Promise<AnimationEventData> {
...
async function loadWeaponCombosDetail(): Promise<Record<string, ComboData[]>> {
  ...
  cachedWeaponCombosDetail = await loadCompressedData<Record<string, ComboData[]>>(
    weaponCombosDetailMsgpackUrl,
    weaponCombosDetailJsonGzippedUrl,
  );
 ...
 */
export async function fetchWeaponCombos(weaponName: string): Promise<ComboData[]> {
